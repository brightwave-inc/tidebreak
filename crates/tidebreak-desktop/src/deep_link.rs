//! `tidebreak://` deep-link handling: gateway pairing.
//!
//! Two link shapes are recognized: `tidebreak://provision?gateway=<url>` and
//! the payload-free `tidebreak://deprovision`. macOS
...
//! ([`tidebreak_server::register_pending_pairing`]), and the in-app sign-in
...
//! ([`tidebreak_server::register_replacing_pairing`]) — the sign-in against
...
//!
//! `tidebreak://deprovision` is the reverse ceremony, with the native
...
//! ([`tidebreak_server::deprovision_provisioned_gateway`]) — anchored to the
...
#[derive(Debug, PartialEq, Eq)]
enum DeepLink {
    /// `tidebreak://provision?gateway=<url>`: park a pairing for sign-in.
    Provision(ProvisionLink),
    /// `tidebreak://deprovision`, payload-free: ask to disconnect from the
...
fn parse_deep_link(url: &tauri::Url) -> Result<DeepLink, String> {
    ...
    // `tidebreak://provision` parses the action as a host; a bare
    // `tidebreak:provision` parses it as the path. Anything left over in the
    ...
    match action.as_str() {
        ...
        _ => Err("not a recognized tidebreak link".into()),
    }
}
...
fn spawn_pairing(app: tauri::AppHandle, link: ProvisionLink) {
    tauri::async_runtime::spawn(async move {
        ...
        match tidebreak_server::register_pending_pairing(&handle, &link.gateway_url).await {
...
fn spawn_deprovision(app: tauri::AppHandle) {
    tauri::async_runtime::spawn(async move {
        ...
        match tidebreak_server::deprovision_target(&handle).await {
            ...
            Ok(DeprovisionTarget::Misconfigured { .. }) => {
                ...
                show_disconnect_failure(
                    &app,
                    &PairingError::Other(tidebreak_core::AgentError::config(
...
async fn confirm_and_replace(
    ...
    match tidebreak_server::register_replacing_pairing(handle, &link.gateway_url, provisioned_url)
        .await
    {
        Ok(PendingRegistration::Registered) => {
            log_pairing(
...
fn repair_prompt(new_origin: &str, old_origin: &str) -> String {
    format!(
        "This device is managed by {old_origin}. Replace it with {new_origin}?\n\n\
         Tidebreak will sign out of {old_origin}, and {new_origin} will control \
...
fn disconnect_prompt(origin: &str) -> String {
    format!(
        "Disconnect this device from {origin}?\n\n\
         Tidebreak will sign out of {origin} and stop being managed by it. \
...
fn disconnect_refusal_message(failure: &PairingError) -> String {
    match failure {
        ...
        PairingError::Conflict { .. } => "The gateway managing this device changed while \
            ...
            .to_string(),
        _ => "Tidebreak could not disconnect this device. Nothing was changed; \
...
fn refusal_message(origin: &str, failure: &PairingError) -> String {
    match failure {
        ...
        PairingError::Conflict { .. } => "The gateway managing this device changed while \
            ...
            .to_string(),
        _ => format!(
            "Tidebreak could not accept the pairing link for {origin}. This \
...
fn log_pairing(app: &tauri::AppHandle, message: &str) {
    use std::io::Write;
    eprintln!("tidebreak-desktop: gateway pairing: {message}");
...
mod tests {
    ...
    fn provision_links_are_held_to_the_contract() {
        let cases: &[(&str, Option<&str>)] = &[
            ...
            (
                "tidebreak://provision?gateway=https://gw.example",
                Some("https://gw.example"),
            ),
            (
                "tidebreak://provision/?gateway=https%3A%2F%2Fgw.example%2F",
                Some("https://gw.example/"),
            ),
            (
                "tidebreak:provision?gateway=https://gw.example",
            ...
            ("tidebreak://settings?gateway=https://gw.example", None),
            ("tidebreak://provision", None),
            ("tidebreak://provision?gateway=", None),
            (
                "tidebreak://provision?gateway=https://a.example&gateway=https://b.example",
                None,
            ),
            (
                "tidebreak://provision?gateway=https://gw.example&extra=1",
                None,
            ),
            (
                "tidebreak://provision/extra?gateway=https://gw.example",
                None,
            ),
            ...
            (
                "tidebreak://user:pw@provision?gateway=https://gw.example",
                None,
            ),
            (
                "tidebreak://provision:9999?gateway=https://gw.example",
                None,
            ),
            ("tidebreak://pro%76ision?gateway=https://gw.example", None),
            ...
            // conflict check).
            ("tidebreak://provision?gateway=notaurl", None),
            ("tidebreak://provision?gateway=ftp://gw.example", None),
            (
                "tidebreak://provision?gateway=https://user:pw@gw.example",
                None,
            ),
            (
                "tidebreak://provision?gateway=https://user@gw.example",
                None,
            ),
            (
                "tidebreak://provision?gateway=https://gw.example/?token=x",
                None,
            ),
            (
                "tidebreak://provision?gateway=https%3A%2F%2Fgw.example%2F%23frag",
                None,
            ),
            ...
            (
                "tidebreak://provision?gateway=http%3A%2F%2F127.0.0.1%3A28081",
                Some("http://127.0.0.1:28081"),
            ),
            (
                "tidebreak://provision?gateway=http%3A%2F%2Flocalhost%3A28081",
                Some("http://localhost:28081"),
            ),
            ("tidebreak://provision?gateway=http://gw.example", None),
            ("tidebreak://provision?gateway=http://10.0.0.5:28081", None),
            (
                "tidebreak://provision?gateway=http://localhost.attacker.example",
                None,
            ),
        ];
    ...
    fn deprovision_links_are_held_to_the_contract() {
        let cases: &[(&str, bool)] = &[
            ("tidebreak://deprovision", true),
            ("tidebreak://deprovision/", true),
            ("tidebreak:deprovision", true),
            ("tidebreak://deprovision?gateway=https://gw.example", false),
            ("tidebreak://deprovision?", false),
            ("tidebreak://deprovision?x=1", false),
            ("tidebreak://user:pw@deprovision", false),
            ("tidebreak://deprovision:9999", false),
            ("tidebreak://deprovision/extra", false),
            ("tidebreak://depro%76ision", false),
    ...
    fn the_dev_scheme_is_a_debug_only_alias() {
        for link in [
            "tidebreak-dev://provision?gateway=https://gw.example",
            "tidebreak-dev://deprovision",
        ] {
            let url = tauri::Url::parse(link).unwrap();
            assert_eq!(
                parse_deep_link(&url).is_ok(),
    ...
    fn the_disconnect_refusal_names_the_right_authority() {
        ...
        let other = disconnect_refusal_message(&PairingError::Other(
            tidebreak_core::AgentError::config("reader failed: token=shh"),
        ));
