...
fn broker_resource(
    root_id: tidebreak_core::HostRootId,
    relative_path: &str,
...
mod tests {
    ...
    fn content_and_failure_results_are_bounded_and_path_free() {
        assert_eq!(
            content_resolution("x".repeat(tidebreak_core::SandboxToolCall::MAX_RESULT_BYTES)),
            failed(DelegatedFileFailureReason::TooLarge)
        );
        assert_eq!(
    ...
    fn exact_server_and_private_receipt_bounds_cover_json_escaping() {
        ...
        if let DelegatedFileResolution::Completed { content } = &ascii {
            ...
            assert!(server.len() <= tidebreak_core::SandboxToolCall::MAX_RESULT_BYTES);
        }
    }
