...
//! OpenWave's code — a self-hosted backend, which per the provisioning
...
//! [`handshake`](openwave_sandbox_protocol::protocol::handshake) documents:
//!
...
//! [`RunInit`](openwave_sandbox_protocol::init::RunInit), so the backend's agent
...
//!
//! - `OPENWAVE_SELFHOST_CONFORMANCE_ENDPOINT` — the `host:port` the backend
//!   listens on, dialable from this machine by the operator's own means.
//! - `OPENWAVE_SELFHOST_CONFORMANCE_SECRET` — the per-run transport secret the
//!   backend was configured with.
//!
//! Run it against the reference implementation (the `openwave-sandbox-agent`
//! binary with `OPENWAVE_TRANSPORT_SECRET` set) or any third-party backend:
//!
//! ```sh
//! OPENWAVE_SELFHOST_CONFORMANCE_ENDPOINT=127.0.0.1:8080 \
//! OPENWAVE_SELFHOST_CONFORMANCE_SECRET=the-per-run-secret \
//! cargo test -p openwave-sandbox-protocol --test selfhost_conformance -- --nocapture
//! ```
...
const ENDPOINT_ENV: &str = "OPENWAVE_SELFHOST_CONFORMANCE_ENDPOINT";
const SECRET_ENV: &str = "OPENWAVE_SELFHOST_CONFORMANCE_SECRET";
