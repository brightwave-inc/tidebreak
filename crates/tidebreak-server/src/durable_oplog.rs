...
//!
//! This backs `tidebreak-sandbox-protocol`'s [`OperationStore`] seam with the
...
//! `tidebreak-core` (the `Store::*_operation` methods), which persist an opaque
//! `(fingerprint, body)` pair keyed by `(run_id, operation_id)` and know nothing
//! of reverse-RPC wire types. This adapter, in `tidebreak-server` — the crate that
...
//! standalone protocol crate keeps no `tidebreak-core` dependency.
//!
...
use tidebreak_core::{OperationClaimOutcome, OperationLogState, OperationLogWrite, Store};
...
fn backend(error: tidebreak_core::AgentError) -> StoreError {
...
mod tests {
    ...
    use tidebreak_sandbox_protocol::reverse::{ModelInferenceParams, ModelInferenceResult};
