...
fn transcript_citation_json_is_closed_and_renderer_bounded() {
    ...
    let snapshot = crate::routes::ChatMessageSnapshot {
        ...
        citations: vec![tidebreak_core::AssistantCitationSnapshot {
            id: tidebreak_core::AssistantCitationId::derive(message_id, 1),
            ordinal: 1,
            document_id: tidebreak_core::DocumentId::new(),
            locator: tidebreak_core::CitationLocator::Pages { start: 2, end: 4 },
        }],
    ...
    async fn stream(&self, _req: ChatRequest) -> Result<BoxStream<'static, ProviderEvent>> {
        Ok(stream::iter(vec![
            ...
            ProviderEvent::Refusal {
                details: tidebreak_core::RefusalDetails::from_category(Some("general_harms")),
            },
        ])
        .boxed())
    }
            ...
            match foreground_call {
                0 => vec![
                    ProviderEvent::ToolCallStarted {
                        index: 0,
                        id: "delegate-1".into(),
                        name: tidebreak_core::SPAWN_SANDBOX_AGENT_TOOL.into(),
                    },
                ...
                3 => {
                    ...
                    vec![
                        ProviderEvent::ToolCallStarted {
                            index: 0,
                            id: "wait-1".into(),
                            name: tidebreak_core::WAIT_FOR_AGENTS_TOOL.into(),
                        },
                        ProviderEvent::ToolCallArgsDelta {
                            index: 0,
...
struct FirstPutGatedBlobStore {
    inner: tidebreak_core::FsBlobStore,
    calls: AtomicUsize,
    entered: Notify,
    release: Notify,
}
...
impl Store for PauseTerminalStore {
    ...
    async fn accept_document_source(
        &self,
        source: &tidebreak_core::DocumentSourceUpsert,
    ) -> Result<tidebreak_core::DocumentRecord> {
    ...
    ) -> Result<tidebreak_core::AcceptTurnOutcome> {
    ...
    ) -> Result<Option<Vec<tidebreak_core::TurnSteer>>> {
    ...
    async fn apply_turn_steer(
        ...
        preceding_citations: &[tidebreak_core::AssistantCitationInput],
        now: chrono::DateTime<chrono::Utc>,
    ) -> Result<Option<tidebreak_core::JournaledTurnSteerOutcome>> {
        let applied = self
            .inner
            .apply_turn_steer(
                turn_id,
    ...
    async fn record_turn_run_failure_and_append_event(
        ...
        lease_token: uuid::Uuid,
        now: chrono::DateTime<chrono::Utc>,
        retry: tidebreak_core::TurnFailureRetry,
        model_steps: i32,
        usage: Usage,
        error_code: &str,
        error_detail: Option<&str>,
    ) -> Result<
        Option<tidebreak_core::JournaledTurnOutcome<tidebreak_core::RecordTurnFailureOutcome>>,
    > {
        if self
    ...
    async fn request_turn_cancellation_and_append_event(
        ...
        Option<
            tidebreak_core::JournaledTurnOutcome<tidebreak_core::RequestTurnCancellationOutcome>,
        >,
    > {
        self.inner
    ...
    async fn finish_turn_cancellation_and_append_event(
        ...
        Option<tidebreak_core::JournaledTurnOutcome<tidebreak_core::FinishTurnCancellationOutcome>>,
    > {
        if self
    ...
    async fn accept_claimed_tool_call(
        ...
        lease_token: uuid::Uuid,
        now: chrono::DateTime<chrono::Utc>,
    ) -> Result<tidebreak_core::AcceptClaimedToolCallOutcome> {
        self.inner
    ...
    async fn request_tool_call_approval(
        &self,
        request: &tidebreak_core::ApprovalRequest,
        requested_at: chrono::DateTime<chrono::Utc>,
    ) -> Result<tidebreak_core::RequestToolApprovalOutcome> {
        self.inner
    ...
    async fn request_tool_call_approval_and_append_event(
        ...
        event_ordinal: i32,
        requested_at: chrono::DateTime<chrono::Utc>,
    ) -> Result<tidebreak_core::JournaledToolApprovalOutcome> {
        self.inner
    ...
    ) -> Result<tidebreak_core::DecideToolApprovalOutcome> {
        self.inner
    ...
    async fn get_tool_call_approval(
        &self,
        call_id: CallId,
    ) -> Result<Option<tidebreak_core::ToolApproval>> {
        ...
        .oneshot(
            Request::builder()
                ...
                .header(
                    ...
                    format!("tidebreak-v1, tidebreak-token.{token}"),
                )
                .header(header::ORIGIN, "https://evil.example")
                .body(Body::empty())
                .unwrap(),
...
/// `tidebreak.lock` on disk must therefore be reclaimable, and the refusal must
