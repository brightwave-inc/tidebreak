...
pub(super) fn implicit_staged_paths(
    ...
    let mut paths = vec![
        "output/.tidebreak-directory".to_owned(),
        "preview/.tidebreak-directory".to_owned(),
    ];
    if with_document_scripts {
        paths.push(DOCUMENT_SCRIPTS_DIR.to_owned());
    }
    if with_skills {
        paths.push(tidebreak_code_execution::SKILLS_DIR.to_owned());
    }
    paths
        .into_iter()
...
) -> Vec<tidebreak_code_execution::HostDep> {
    let mut required: Vec<tidebreak_code_execution::HostDep> = Vec::new();
    for skill in skills {
        let implied =
            (!skill.package.npm_deps.is_empty()).then_some(tidebreak_code_execution::HostDep::Node);
...
pub(super) async fn prepare_execution_directories(
    ...
    for name in ["output", "preview", "documents"] {
        ...
        if mirrored {
            ...
            directory
                .write_file(".tidebreak-directory", &[])
                .await
