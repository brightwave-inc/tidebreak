...
//! directly and refreshes only open identities whose live tier has aged out,
...
/// One reconcile pass: group every open fact row by owner, then refresh only
...
/// an existing row neither enumerates repositories nor writes a list snapshot
/// back through `save_pull_request_fact`.
