...
async fn read_config(store: &dyn Store) -> tidebreak_core::Result<VoiceTranscriptionConfig> {
...
async fn transcribe_gemini(
    ...
    let provider = match credential {
        Some(ProviderCredential::ApiKey { key }) if !key.is_empty() => {
            tidebreak_router::GeminiProvider::new(key)
        }
        Some(_) => {
            return Err(ServerError::conflict(
...
mod tests {
    ...
    use std::sync::Mutex;
    use tidebreak_core::{DbStore, Result as CoreResult};
