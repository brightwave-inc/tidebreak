...
fn command_prefix_scope(
    action: &tidebreak_core::ToolActionPreview,
    tokens: usize,
) -> Option<GrantScope> {
    tidebreak_core::GrantScope::ladder_for_action(action)
        .into_iter()
        .find(|scope| {
...
mod tests {
    ...
    async fn setup_with_arguments(
        ...
        (
            ...
            ApprovalRequest {
                ...
                kind: tidebreak_core::ToolApprovalKind::for_tool_name(tool_name),
                preview: None,
            },
        )
    }
    ...
    async fn judge_verdicts_are_a_cas_the_human_always_wins() {
        ...
        let declined = store
            ...
            .unwrap()
            .unwrap();
        assert_eq!(declined.status, tidebreak_core::ToolApprovalStatus::Pending);
        assert_eq!(
            declined.auto_judge_status,
            Some(tidebreak_core::AutoJudgeStatus::Declined)
        );
        assert!(!broker
    ...
    async fn a_judge_approval_resolves_the_parked_decision() {
        ...
        assert_eq!(
            ...
            Some(tidebreak_core::AutoJudgeStatus::Approved)
        );
    }
    ...
    async fn setup_in_project(
        arguments: serde_json::Value,
    ) -> (Arc<dyn Store>, tidebreak_core::ProjectId, ApprovalRequest) {
        let db = tempfile::tempdir().unwrap();
        let store: Arc<dyn Store> = Arc::new(
            DbStore::connect(&format!(
                "sqlite://{}?mode=rwc",
    ...
    async fn a_grant_made_in_a_project_covers_the_next_chat_in_it() {
        ...
        assert!(matches!(
            ...
            tidebreak_core::ApprovalRequiredPublication::StandingGrant
        ));
        assert_eq!(registration.decision.await, ApprovalDecision::Approve);
    ...
    async fn a_web_extract_grant_is_stored_under_its_own_kind_and_covers_the_page() {
        ...
        assert_eq!(
            grants[0].grant.kind(),
            tidebreak_core::ToolApprovalKind::WebExtractMayFetchUrl
        );
        ...
        for rung in [
            ...
            assert_eq!(
                store
                    ...
                    .unwrap()
                    .status,
                tidebreak_core::ToolApprovalStatus::Pending
            );
        }
    ...
    async fn a_parked_create_app_call_can_be_approved() {
        ...
        assert_eq!(
            request.kind,
            tidebreak_core::ToolApprovalKind::WorkspaceMayModifyFiles
        );
        let broker = ApprovalBroker::new(store);
        let _pending = broker.register(request.clone(), None).await;
        assert_eq!(
