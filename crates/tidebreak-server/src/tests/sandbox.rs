...
async fn agent_run_activity_history_is_ordered_typed_and_names_submitted_files() {
    ...
    let exec = SandboxToolCallRequest {
        ...
        name: tidebreak_core::SANDBOX_EXEC_TOOL.into(),
        arguments: serde_json::json!({
            "command": "python3",
    ...
    let submitted = tidebreak_core::OutputId::new();
    store
        .create_output(&tidebreak_core::CreateOutput {
            id: submitted,
            chat_id: chat.id,
    ...
    store
        .submit_agent_run_submission(
            run.id,
            completion_lease,
            &[tidebreak_core::AgentRunSubmittedOutput {
                output_id: submitted,
                filename: "Q3 revenue.md".into(),
            }],
...
async fn delegated_file_routes_are_native_only_and_expose_only_exact_broker_authority() {
    ...
    let resource = tidebreak_core::SandboxAgentFileResource {
        root_id,
    ...
    let child = match store
        ...
        .unwrap()
    {
        tidebreak_core::CheckpointSandboxSpawnOutcome::Checkpointed { child, .. } => child,
...
async fn sandbox_cancel_route_is_authenticated_closed_and_idempotent() {
    ...
    let foreground = post_json(
        ...
        &format!(
            ...
            chat.id,
            tidebreak_core::AgentRunId::foreground_for_chat(chat.id)
        ),
        serde_json::json!({}),
    )
    .await;
...
async fn sandbox_cancel_route_rejects_completed_and_failed_runs() {
    ...
    for attempt in 1..=tidebreak_core::AgentRun::DEFAULT_MAX_ATTEMPTS {
        let lease = uuid::Uuid::new_v4();
        assert_eq!(
            store
        ...
        if attempt < tidebreak_core::AgentRun::DEFAULT_MAX_ATTEMPTS {
...
async fn sandbox_cancel_route_signals_only_the_durable_run_receipt() {
    ...
    let unrelated_run = tidebreak_core::AgentRunId::sandbox_for_spawn_call(CallId::new());
    let unrelated = state
        .sandbox_attempts
...
async fn agent_run_list_omits_foreground_client_activity() {
    ...
    let historical = match store.accept_tool_call(&historical).await.unwrap() {
        tidebreak_core::AcceptToolCallOutcome::Accepted(call) => call,
