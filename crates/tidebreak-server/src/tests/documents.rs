...
async fn document_file_content_preserves_document_scope_before_blob_access() {
    ...
    .await;
    let project_document_id: tidebreak_core::DocumentId = project_document["document_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    .await;
    let chat_document_id: tidebreak_core::DocumentId = chat_document["document_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
...
async fn raw_ingest_retains_exact_bytes_and_decodes_before_responding() {
    ...
    assert_eq!(
        document_id,
        tidebreak_core::DocumentId::derive("file:///raw.txt")
    );
    ...
    let blobs = tidebreak_core::FsBlobStore::new(dir.path().join("blobs"));
...
async fn streamed_raw_ingest_accepts_full_size_chunked_sources_and_deduplicates_by_content() {
    ...
    let source_blob = tidebreak_core::DocumentBlob::from_bytes(&raw);
    ...
    assert_eq!(
        document_id,
        tidebreak_core::DocumentId::derive_for_chat_content(chat.id, source_blob.sha256)
    );
    assert_eq!(
        tidebreak_core::FsBlobStore::new(dir.path().join("blobs"))
            .get(source_blob.id)
    ...
    assert_eq!(
        store
            .list_document_ids(tidebreak_core::DocumentScope::Chat(chat.id))
            .await
            .unwrap(),
        vec![document_id]
    );
...
async fn project_document_routes_enforce_corpus_identity_and_ownership() {
    ...
    assert_eq!(
        root["document_id"],
        tidebreak_core::DocumentId::derive(uri).to_string()
    );
    assert_eq!(
        a["document_id"],
        tidebreak_core::DocumentId::derive_for_project(project_a.id, uri).to_string()
    );
    assert_eq!(
        b["document_id"],
        tidebreak_core::DocumentId::derive_for_project(project_b.id, uri).to_string()
    );
...
async fn chat_document_routes_isolate_sources_and_delete_lifecycle() {
    ...
    .await;
    let first_id: tidebreak_core::DocumentId = first_ingest["document_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    let second_id: tidebreak_core::DocumentId = second_ingest["document_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
...
async fn document_catalog_pages_metadata_and_keeps_project_content_private() {
    ...
    let project = make_project(&router, &bearer).await;
    let project_document_id = tidebreak_core::DocumentId::new();
    let now = chrono::Utc::now();
    store
        .create_document(&tidebreak_core::DocumentRecord {
            chat_id: None,
    ...
    assert_eq!(
        get(format!("/documents/{}", tidebreak_core::DocumentId::new()))
            .await
            .status(),
        StatusCode::NOT_FOUND
    );
...
async fn document_catalog_cursor_preserves_nanosecond_ordering() {
    ...
    for nanos in [900, 800, 700] {
        let id = tidebreak_core::DocumentId::new();
...
async fn concurrent_same_document_ingests_are_last_write_wins() {
    ...
    let blobs = Arc::new(FirstPutGatedBlobStore {
        inner: tidebreak_core::FsBlobStore::new(dir.path().join("blobs")),
    ...
    let record = store
        .get_document(tidebreak_core::DocumentId::derive("file:///concurrent.txt"))
        .await
        .unwrap()
        .unwrap();
...
async fn ingest_accepts_any_media_type_via_the_fallback_parser() {
    ...
    let textual_id: tidebreak_core::DocumentId = json_body::<serde_json::Value>(textual).await
        ["document_id"]
        .as_str()
        .unwrap()
        .parse()
    ...
    let binary_id: tidebreak_core::DocumentId = json_body::<serde_json::Value>(binary).await
        ["document_id"]
        .as_str()
        .unwrap()
        .parse()
    ...
    assert_eq!(
        tidebreak_core::DocumentReadiness::of(binary_doc.is_readable()),
        tidebreak_core::DocumentReadiness::StoredNoText
    );
}
...
async fn agent_deps_registers_server_tools_and_closed_foreground_capabilities() {
    ...
    assert!(system_prompt.contains("You are Tidebreak"));
    ...
    assert!(
        names
            .iter()
            .any(|n| n == tidebreak_code_execution::EXEC_TOOL_NAME),
    ...
    assert_eq!(
        surface
            .tools
            .execution(tidebreak_core::REQUEST_FOLDER_ACCESS_TOOL),
        Some(ToolCallExecution::Client)
    );
    ...
    assert_eq!(
        surface
            .tools
            .execution(tidebreak_core::IMPORT_CONNECTED_FILE_TOOL),
        Some(ToolCallExecution::Client)
    );
    assert!(surface
        .tools
        .get(tidebreak_core::IMPORT_CONNECTED_FILE_TOOL)
        .is_none());
    assert!(foreground.contains(tidebreak_core::IMPORT_CONNECTED_FILE_TOOL));
    ...
    assert!(surface
        .tools
        .get(tidebreak_core::REQUEST_FOLDER_ACCESS_TOOL)
        .is_none());
    let spec = surface
        .tools
        .specs()
        .into_iter()
        .find(|spec| spec.name == tidebreak_core::REQUEST_FOLDER_ACCESS_TOOL)
        .expect("folder consent tool is advertised");
    assert_eq!(spec, tidebreak_core::request_folder_access_tool_spec());
    assert!(surface.tools.client_arguments_are_valid(
...
async fn promoting_a_document_shares_it_with_the_project_and_keeps_the_original() {
    ...
    assert_eq!(
        ...
        tidebreak_core::DocumentId::derive_for_project(project.id, uri).to_string()
    );
