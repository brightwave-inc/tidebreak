...
async fn chat_settings_stick_to_the_next_chat() {
    ...
    assert_eq!(
        ...
        tidebreak_core::NetworkPolicy::PackageManagers
    );
    ...
    assert_eq!(explicit.network_policy, tidebreak_core::NetworkPolicy::Off);
...
async fn chat_network_policy_defaults_open_and_persists_normalized_exact_hosts() {
    ...
    let bearer = format!("Bearer {token}");
    let chat = make_chat(&router, &bearer).await;
    assert_eq!(chat.network_policy, tidebreak_core::NetworkPolicy::Open);
    ...
    assert_eq!(
        ...
        tidebreak_core::NetworkPolicy::AllowedHosts {
...
async fn a_retained_result_projection_survives_reopening_the_chat() {
    ...
    store
        ...
        .await
        .unwrap();
    let preview = tidebreak_core::ToolResultPreview::Entries {
        entries: vec![tidebreak_core::ResultEntry::new(
            tidebreak_core::ResultEntryKind::File,
            "q3.md",
        )],
        failures: vec![tidebreak_core::ResultFailure::new("q4.md", "unreadable")],
        elided: 0,
    };
    assert_eq!(
        store
        ...
        store
            .resolve_server_tool_call(
                ...
                &ToolCallResolution::Failed {
                    ...
                    error_code: tidebreak_core::ToolErrorCategory::ConfigurationRequired
                        .as_str()
                        .into(),
                    error_detail: None,
                },
    ...
    assert!(activity.iter().any(|card| {
        card["tool"] == "spawn_sandbox_agent"
            && card["background_agent_run_id"]
                == serde_json::json!(tidebreak_core::AgentRunId::sandbox_for_spawn_call(
                    spawn_call_id
                ))
    }));
    assert!(activity.iter().any(|card| {
...
async fn a_managed_ceiling_locks_over_ceiling_permission_modes() {
    ...
    impl crate::managed_policy::OsPolicySource for CappedAtAsk {
        ...
        fn permission_mode_ceiling(
            &self,
        ) -> tidebreak_core::Result<Option<tidebreak_core::PermissionMode>> {
            Ok(Some(tidebreak_core::PermissionMode::Ask))
        }
    }
