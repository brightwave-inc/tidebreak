...
impl GatewayDraftClient for FakeGateway {
    ...
    ) -> tidebreak_core::Result<Option<GatewayRegistrationOutcome>> {
    ...
    async fn consent(
        ...
        shared_app_id: &str,
        revision_id: &str,
    ) -> tidebreak_core::Result<Option<GatewayConsentOutcome>> {
        self.calls.lock().unwrap().push(format!(
...
impl Fixture {
    ...
    async fn held(
        &self,
        gateway_base_url: &str,
    ) -> Option<tidebreak_core::local_app::AppGatewayDraft> {
        self.store
...
async fn a_grant_survives_a_registration_the_gateway_refuses() {
    ...
    impl GatewayDraftSource for UnreachableGateway {
        async fn ensure_registered(
            ...
            Err(tidebreak_core::AgentError::Store(
