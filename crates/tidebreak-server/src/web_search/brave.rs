...
mod tests {
    ...
    const RESPONSE: &[u8] = br#"{
      "type": "search",
      "query": { "original": "tidebreak", "more_results_available": true },
    ...
    async fn maps_the_web_cluster_and_sends_the_documented_query() {
        ...
        let response = provider
            .search(WebSearchRequest::new("tidebreak", 3).unwrap())
            .await
            .unwrap();
        ...
        assert_eq!(first.title, "Tidebreak release notes");
    ...
    async fn domain_filters_become_operators_and_a_hard_result_filter() {
        ...
        let query = sent
            ...
            .1
            .clone();
        assert_eq!(query, "tidebreak site:example.com");
    }
