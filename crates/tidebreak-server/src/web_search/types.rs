...
pub const MAX_QUERY_CHARS: usize = tidebreak_core::MAX_WEB_SEARCH_QUERY_CHARS;
/// A provider request may ask for no more than this many normalized results.
pub const MAX_RESULTS: usize = tidebreak_core::MAX_WEB_SEARCH_RESULTS;
/// A request may restrict results to no more than this many domains.
pub const MAX_DOMAINS: usize = tidebreak_core::MAX_WEB_SEARCH_DOMAINS;
...
impl WebSearchProviderKind {
    ...
    /// Stable key in the application [`SecretProvider`](tidebreak_core::SecretProvider).
    ///
...
const _: () =
    assert!(tidebreak_core::MAX_WEB_EXTRACT_URL_BYTES == super::fetch_policy::MAX_FETCH_URL_BYTES);
