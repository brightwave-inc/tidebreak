...
pub(super) fn delegated_file_admission_matches(
    run: &AgentRun,
    admission: &tidebreak_core::SandboxAgentAdmission,
...
pub(super) fn classify_sandbox_tool_call(
    ...
    let valid = match name.as_str() {
        ...
        tidebreak_core::SANDBOX_DONE_TOOL => {
            tidebreak_core::validate_sandbox_done_arguments(&arguments)
                && serde_json::from_value::<tidebreak_core::SandboxDoneArgs>(arguments.clone())
                    .is_ok()
        }
