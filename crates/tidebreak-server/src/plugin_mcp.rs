...
/// Longest runtime server name, mirroring [`tidebreak_mcp::MAX_SERVER_NAME_BYTES`].
const MAX_RUNTIME_NAME_BYTES: usize = tidebreak_mcp::MAX_SERVER_NAME_BYTES;
...
pub(crate) const SSE_UNSUPPORTED_DIAGNOSTIC: &str =
    "This plugin server uses the legacy SSE transport, which Tidebreak does not implement.";
...
fn definition_for(
    ...
    server: &tidebreak_code_execution::McpServer,
    name: &str,
    root: &str,
    data: &str,
) -> McpServerDefinition {
...
mod tests {
    use super::*;
    use tidebreak_code_execution::{McpHttpServer, McpServer, McpStdioServer, PluginMcpConfig};
