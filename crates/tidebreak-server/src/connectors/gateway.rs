...
impl GatewayAuthConfig {
    /// The registered `tidebreak` public client: the gateway's client registry
    /// names it on the consent page and attributes its sessions to Tidebreak.
    ...
    pub fn new(base_url: &str) -> Result<Self> {
        Self::with_client(base_url, "tidebreak", Some("tidebreak".to_string()))
    }
...
/// ([`tidebreak_core::local_app::MAX_GATEWAY_APP_ID_BYTES`]): 1–128 bytes of
/// printable, non-whitespace ASCII. The id is the gateway's and Tidebreak never
...
fn validate_gateway_app_id(app_id: &str) -> Result<()> {
    ...
    if app_id.is_empty()
        || app_id.len() > tidebreak_core::local_app::MAX_GATEWAY_APP_ID_BYTES
