...
fn parse_command_list(stdout: &[u8]) -> Vec<HarnessCommand> {
    String::from_utf8_lossy(stdout)
        ...
        .into_iter()
        .flatten()
        .filter_map(|name| name.as_str().map(str::trim).map(str::to_owned))
...
pub(crate) mod tests {
    ...
    fn capabilities_for_2_1_233_are_honest() {
        ...
        assert_eq!(
            ClaudeCodeAdapter::new().capabilities(&listed).slash_commands,
            CapLevel::Supported
        );
        listed.commands.clear();
    }
