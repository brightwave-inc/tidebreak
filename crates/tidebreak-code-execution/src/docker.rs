...
//! sandbox-agent container backend in `tidebreak-server`. Foreground exec is
...
/// unprivileged user and declares it as `TIDEBREAK_SANDBOX_WORKSPACE`.
...
const CONTAINER_PREFIX: &str = "tidebreak-exec-";
...
/// enumerate them with `docker ps --filter label=tidebreak.exec-workspace`.
const WORKSPACE_LABEL: &str = "tidebreak.exec-workspace";
...
const CONFIGURATION_LABEL: &str = "tidebreak.exec-configuration";
...
impl DockerExecutionProvider {
    ...
    /// remembers creating. A host that restarted — or a second Tidebreak
...
mod tests {
    ...
    fn every_container_is_confined_and_carries_no_host_mount() {
        ...
        assert!(args
            .windows(2)
            .any(|pair| pair == ["--label", "tidebreak.exec-workspace=chat-123"]));
    ...
    /// both Tidebreak's workspace label and this provider configuration may be
    /// adopted; legacy/mismatched Tidebreak containers are replaceable, while
    ...
    ///
    /// ```text
    /// cargo test -p tidebreak-code-execution -- --ignored docker_container
    /// ```
    #[tokio::test]
