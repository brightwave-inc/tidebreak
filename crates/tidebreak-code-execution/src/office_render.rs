...
/// `output/reports/deck.pptx` is `.tidebreak/render/reports/deck.pptx.pdf` —
/// the full source filename stays in the name so two sources with one stem
/// can never answer for each other.
pub const OFFICE_RENDER_DIR: &str = ".tidebreak/render";
        ...
        match render_candidate(converter, host_dir, &candidate).await {
            Ok(()) => notes.push(format!(
                "office render: {} -> {OFFICE_RENDER_DIR}/{}.pdf (converted on the host); \
                 render page images with .tidebreak/exec-scripts/render_pdf.py — on a managed \
...
mod tests {
    ...
    async fn converts_new_office_outputs_once_and_notes_the_staged_path() {
        ...
        assert_eq!(
            std::fs::read(host.path().join(".tidebreak/render/deck.pptx.pdf")).unwrap(),
            b"%PDF(pptx)deck bytes"
        );
        assert_eq!(
            std::fs::read(host.path().join(".tidebreak/render/reports/brief.docx.pdf")).unwrap(),
            b"%PDF(docx)doc bytes"
        );
        assert!(!host.path().join(".tidebreak/render/data.csv.pdf").exists());
        assert_eq!(notes.len(), 2, "{notes:?}");
        assert!(
            notes[0].contains("output/deck.pptx")
                && notes[0].contains(".tidebreak/render/deck.pptx.pdf")
    ...
    async fn degraded_states_produce_one_honest_note() {
        ...
        assert!(!host.path().join(".tidebreak/render").exists());
    }
    ...
    async fn symlinked_sources_and_targets_are_refused() {
        ...
        std::fs::create_dir_all(host.path().join(".tidebreak")).unwrap();
        std::os::unix::fs::symlink(outside.path(), host.path().join(".tidebreak/render")).unwrap();
        let notes = render_office_outputs(Some(&converter), host.path()).await;
        assert!(
            notes
                .iter()
