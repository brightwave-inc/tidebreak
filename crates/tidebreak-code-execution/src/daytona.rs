...
/// organization for the official Tidebreak documents image.
///
...
const DOCUMENTS_SNAPSHOT: &str = "tidebreak-documents-main-20260903-3e41563-r1212-exec";
...
/// Fixed key for Daytona credentials in Tidebreak's host secret store.
...
impl DaytonaExecutionProvider {
    ...
    /// instead of the official Tidebreak documents snapshot.
    ///
    ...
    async fn create_sandbox(&self, workspace_id: &str) -> Result<RemoteSession, ExecError> {
        ...
        let response = self
            ...
            .json(&CreateSandboxRequest {
                labels: HashMap::from([
                    ("tidebreak_workspace_id", workspace_id),
        ...
        match outcome {
            ...
            Err(SnapshotError::Degraded(reason)) => {
                tracing::warn!(
                    ...
                    "could not prepare the Tidebreak documents image in Daytona ({reason}); \
        ...
        match self.fetch_snapshot(DOCUMENTS_SNAPSHOT).await? {
            ...
            None => {
                tracing::info!(
                    ...
                    "preparing the Tidebreak documents sandbox image in Daytona \
        ...
        loop {
            match snapshot.state.as_deref() {
                ...
                Some("inactive") if !requested_activation => {
                    tracing::info!(
                        snapshot = DOCUMENTS_SNAPSHOT,
                        "reactivating the Tidebreak documents snapshot in Daytona"
                    );
                    preparing.announce();
                    let response = self
                        .client
            ...
            if reported.elapsed() >= DAYTONA_SNAPSHOT_PROGRESS_INTERVAL {
                tracing::info!(
                    ...
                    "still preparing the Tidebreak documents sandbox image in Daytona"
                );
...
struct CreateSnapshotRequest<'a> {
    ...
    /// snapshot. Required for the Tidebreak documents image — see
...
mod tests {
    ...
    fn request(execution_id: &str) -> ExecRequest {
        ExecRequest::new(
            ...
            vec![
                "%s".into(),
                "a; touch /tmp/not-tidebreak".into(),
                "single'quote".into(),
            ],
            ".",
        )
    ...
    fn direct_argv_is_quoted_before_crossing_daytonas_shell_api() {
        assert_eq!(
            ...
            "exec 'printf' '%s' 'a; touch /tmp/not-tidebreak' 'single'\"'\"'quote'"
        );
    }
    ...
    async fn daytona_reuses_the_chat_sandbox_and_replays_an_exact_execution() {
        ...
        assert_eq!(create_body["labels"]["tidebreak_workspace_id"], "chat-123");
    ...
    async fn daytona_registers_the_documents_snapshot_into_the_account_on_first_use() {
        ...
        // An account that has never seen Tidebreak: the snapshot is missing, so
