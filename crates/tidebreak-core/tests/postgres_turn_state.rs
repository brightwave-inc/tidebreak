...
async fn postgres_concurrent_exact_nonblocking_spawn_checkpoint_converges_once() {
    ...
    request.remaining_requests = vec![tidebreak_core::SandboxAgentSpawnRequest {
...
async fn postgres_parent_completion_and_child_admission_form_one_terminal_boundary() {
    ...
    match completion {
        CompleteTurnRunOutcome::Completed(_) => {
            assert!(matches!(
                admission,
                tidebreak_core::AdmitSandboxAgentRunOutcome::ParentUnavailable
                    | tidebreak_core::AdmitSandboxAgentRunOutcome::LeaseLost
            ));
        ...
        CompleteTurnRunOutcome::ChildrenOutstanding { child_run_ids, .. } => {
            assert!(matches!(
                admission,
                tidebreak_core::AdmitSandboxAgentRunOutcome::Accepted { .. }
            ));
...
async fn postgres_agent_run_identity_collision_recovers_exactly_across_chats() {
    ...
    for task in tasks {
        match task.await.unwrap() {
            Ok(Some(tidebreak_core::AdmitSandboxAgentRunOutcome::Accepted { .. })) => accepted += 1,
            Ok(Some(tidebreak_core::AdmitSandboxAgentRunOutcome::IdentityConflict)) => {
                conflicted += 1
            }
...
async fn postgres_concurrent_exact_sandbox_admission_converges() {
    ...
    let existing = outcomes
        ...
        .filter(|outcome| {
            matches!(
                outcome.as_ref().unwrap(),
                tidebreak_core::AdmitSandboxAgentRunOutcome::Existing { .. }
            )
        })
        .count();
...
async fn postgres_sandbox_admission_checks_lease_time_after_lock_wait() {
    ...
    assert!(matches!(
        ...
        Some(tidebreak_core::AdmitSandboxAgentRunOutcome::LeaseLost)
    ));
    assert!(store
...
async fn postgres_project_deletion_serializes_with_staged_source_ingestion() {
    ...
    let source = DocumentSourceUpsert {
        ...
        chat_id: None,
        origin_uri: Some("file:///postgres-project-race.bin".into()),
        media_type: "application/octet-stream".into(),
        title: None,
        source_blob: DocumentBlob::from_digest([0x7a; 32], 1),
