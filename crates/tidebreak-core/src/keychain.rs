...
const DEFAULT_SERVICE: &str = "tidebreak";
...
impl KeychainSecretProvider {
    /// Use the default service name (`tidebreak`).
    ///
    ...
    /// **debug builds only**, the `TIDEBREAK_KEYCHAIN_MOCK` env var (any
    ...
    /// Honors [`use_mock`](Self::use_mock) and `TIDEBREAK_KEYCHAIN_MOCK` the
    ...
    pub fn with_service(service: impl Into<String>) -> Self {
        ...
        if std::env::var("TIDEBREAK_KEYCHAIN_MOCK").is_ok_and(|v| !v.is_empty()) {
            Self::use_mock();
        }
        Self {
...
mod tests {
    ...
    async fn the_mock_stores_what_it_is_given() {
        ...
        let secrets = KeychainSecretProvider::with_service("tidebreak-test");
        ...
        let other = KeychainSecretProvider::with_service("tidebreak-test-other");
    ...
    async fn native_credential_store_round_trip() {
        ...
        let secrets = KeychainSecretProvider::with_service("tidebreak-ci-probe");
