...
pub fn is_auto_judge_candidate(
    ...
    match &action {
        ToolActionPreview::Exec { command, args, .. } => {
            ...
            tidebreak_shell_policy::analyze_argv(&exec_argv(command, args), &broadest).verdict
...
impl GrantScope {
    ...
    fn covers_argv(&self, action: &ToolActionPreview, command: &str, args: &[String]) -> bool {
        ...
        let rule = match self {
            Self::WholeTool => tidebreak_shell_policy::CommandRule::new(
                tidebreak_shell_policy::CommandRuleKind::All,
                Vec::new(),
            ),
            Self::AnyArgsFor { command } => tidebreak_shell_policy::CommandRule::new(
                tidebreak_shell_policy::CommandRuleKind::Prefix,
                vec![command.clone()],
            ),
            Self::CommandPrefix { tokens } => tidebreak_shell_policy::CommandRule::new(
                tidebreak_shell_policy::CommandRuleKind::Prefix,
                tokens.clone(),
            ),
            // The projection already matched above, so the analyzer is asked
            // only whether this invocation clears the floor.
            Self::ExactAction(_) => tidebreak_shell_policy::CommandRule::new(
                tidebreak_shell_policy::CommandRuleKind::Exact,
        ...
        let ruleset = tidebreak_shell_policy::ShellRuleSet {
            allow: vec![rule],
            deny: Vec::new(),
        };
        tidebreak_shell_policy::analyze_argv(&exec_argv(command, args), &ruleset).verdict
            == tidebreak_shell_policy::ShellVerdict::Allow
    }
    ...
    pub fn ladder_for_action(action: &ToolActionPreview) -> Vec<Self> {
        ...
        if let ToolActionPreview::Exec { command, args, .. } = &action {
            ...
            let mut ladder: Vec<Self> = tidebreak_shell_policy::suggested_rungs_for_argv(&argv)
                .into_iter()
                .map(|rule| match rule.kind {
                    tidebreak_shell_policy::CommandRuleKind::Exact => {
                        Self::ExactAction(action.clone())
                    }
                    tidebreak_shell_policy::CommandRuleKind::Prefix => Self::CommandPrefix {
                        tokens: rule.tokens,
                    },
                    tidebreak_shell_policy::CommandRuleKind::All => Self::WholeTool,
                })
                .collect();
