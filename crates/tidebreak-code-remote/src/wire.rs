...
pub enum SupervisorMessageBody {
    ...
    Input(String),
    /// A protected native-tool result from the server.
    Tool(SupervisorToolResult),
}

/// One native-tool result returned by the server.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
...
pub struct SupervisorToolResult {
    ...
    /// Bounded artifacts to materialize into sandbox scratch. A server-only
    /// path never pretends to work remotely: the agent writes these bytes and
    /// then the engine sees real files.
    #[serde(default)]
    pub artifacts: Vec<SupervisorArtifact>,
}

/// One bounded artifact materialized into sandbox scratch.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
pub struct SupervisorArtifact {
    ...
    /// File bytes, bounded by the host (both text/JSONL and images at 2 MiB).
    pub bytes: Vec<u8>,
}
