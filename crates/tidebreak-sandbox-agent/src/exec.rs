...
//! adapter bounds its confined child (`tidebreak-code-execution`'s local
...
//! network topology (`tidebreak-server`'s `sandbox_docker`): the container's
...
const DOCUMENT_SCRIPTS_DIR: &str = "/opt/tidebreak/exec-scripts";
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::for_args::<ExecArgs>(
            ...
            "Run a shell command inside the sandbox workspace and return its \
             ...
             cap. Document helpers live at $TIDEBREAK_EXEC_SCRIPTS and write visual \
             results to preview/; overview/grid/thumbnail names are reviewed first. \
             Examples: python3 $TIDEBREAK_EXEC_SCRIPTS/render_pdf.py report.pdf \
             --pages 1-2; python3 $TIDEBREAK_EXEC_SCRIPTS/analyze_xlsx.py budget.xlsx.",
        )
    }
...
async fn run_bounded(
    ...
    cmd.arg("-c")
        ...
        .env("PATH", FIXED_PATH)
        .env("TIDEBREAK_EXEC_SCRIPTS", DOCUMENT_SCRIPTS_DIR)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true);
...
mod tests {
    ...
    async fn exposes_document_helpers_and_workspace_conventions() {
        ...
        let tool = ExecTool::new(dir.path(), DEFAULT_EXEC_TIMEOUT);
        let spec = tool.spec();
        assert!(spec.description.contains("TIDEBREAK_EXEC_SCRIPTS"));
        ...
        let out = tool
            .execute(
                &ctx(),
                serde_json::json!({
                    "command": "printf '%s' \"$TIDEBREAK_EXEC_SCRIPTS\""
                }),
            )
            .await
            .unwrap();
    ...
    async fn the_egress_proxy_survives_the_cleared_environment() {
        ...
        let outcome = tokio::time::timeout(
            ...
            run_bounded(
                ...
                &[("HTTPS_PROXY", "http://tidebreak-egress:8118".to_owned())],
            ),
        )
        .await
        .expect("exec returned within the outer bound");
        assert!(
            outcome.stdout.contains("http://tidebreak-egress:8118"),
            "{}",
            outcome.render()
        );
    }
