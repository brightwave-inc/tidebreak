...
use openwave_code_execution::{ExecutionWorkspaceId, WorkspaceLifecycle};
use openwave_core::{AgentError, AgentRunId, Result, Store};
            ...
            let delay = match self.sweep_once().await {
                Ok(report) => {
                    if report.reaped > 0 || !report.failures.is_empty() {
                        eprintln!(
                            "openwave: agent-run scratch sweep scanned {}, reaped {}, skipped {}, remaining {}, errors {}",
                            report.scanned,
                            report.reaped,
                            report.skipped,
                            report.remaining,
                    ...
                    for failure in &report.failures {
                        match failure.run_id {
                            Some(run_id) => eprintln!(
                                "openwave: agent run {run_id} scratch reap failed: {}",
                                failure.error
                            ),
                            None => eprintln!(
                                "openwave: agent-run scratch scan failed: {}",
                                failure.error
                            ),
                        }
                    }
                ...
                Err(error) => {
                    eprintln!("openwave: agent-run scratch sweep failed: {error}");
