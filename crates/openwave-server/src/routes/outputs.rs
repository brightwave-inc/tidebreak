...
//! reading `openwave-core`'s `Store` directly, which made every one of them
...
pub const MAX_OUTPUT_REVISION_BODY_BYTES: usize = 2 * openwave_core::MAX_DELIVERABLE_BYTES;
...
pub async fn get_chat_output_content(
    ...
    Ok((
        [
            ...
            (
                header::HeaderName::from_static("x-openwave-revision-id"),
