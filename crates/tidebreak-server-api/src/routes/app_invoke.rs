...
use tidebreak_core::local_app::{AppBinding, AppGrantBinding, AppRecord, AppRevision};
...
pub struct AppInvokeRequest {
    ...
    pub folder: Option<tidebreak_core::id::HostRootId>,
...
enum InvokeSurface {
    ...
    Folder {
        folder: tidebreak_core::id::HostRootId,
        path: String,
        op: FolderOp,
    },
}
        ...
        .any(|binding| match binding {
            AppBinding::Folder(binding) => {
                binding.folder == folder
                    && (!writes
                        || binding.access == tidebreak_core::local_app::FolderAccess::ReadWrite)
            }
            ...
            matches!(
                ...
                AppGrantBinding::Folder(granted)
                    if granted.folder == *folder
                        && (!writes
                            || granted.access
                                == tidebreak_core::local_app::FolderAccess::ReadWrite)
            )
        }),
