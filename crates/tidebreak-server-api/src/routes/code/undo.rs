...
use crate::code::checkpoint::{ChangedFile, HunkSelector, RevertedChange};
...
use super::types::{
    ...
    CodeFileChange, CodeWorktreeChange, DiscardWorkspaceChangesBody, RestoreCheckpointBody,
    RevertWorkspaceChangeBody,
};
...
/// on: `409 worktree_changed` (the worktree moved after the preview) and
...
pub async fn restore_checkpoint(
    ...
    let restored = code
        .restore_checkpoint(id, body.target, body.expected_tree.as_deref())
        .await?;
...
/// `409 revert_conflict` means the worktree no longer holds the change.
pub async fn revert_workspace_change(
    code: ScopedCode,
    Path(id): Path<WorkspaceId>,
    Json(body): Json<RevertWorkspaceChangeBody>,
) -> Result<Json<CodeWorktreeChange>, ServerError> {
    let hunk = body.hunk.as_ref().map(|hunk| HunkSelector {
        index: usize::try_from(hunk.index).unwrap_or(usize::MAX),
        text: &hunk.text,
    });
    let reverted = code
...
/// `409 no_change` and changes nothing.
pub async fn discard_workspace_changes(
    code: ScopedCode,
    Path(id): Path<WorkspaceId>,
    Json(body): Json<DiscardWorkspaceChangesBody>,
) -> Result<Json<CodeWorktreeChange>, ServerError> {
    let discarded = code.discard_workspace_changes(id, &body.paths).await?;
