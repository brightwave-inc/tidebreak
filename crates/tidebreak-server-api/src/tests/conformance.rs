...
async fn self_host_app() -> (Router, AppState, Arc<dyn Store>, tempfile::TempDir) {
    ...
    let mut runtime =
        CodeRuntime::with_registry(db.clone(), dir.path().to_path_buf(), AdapterRegistry::new());
    runtime
        .adapters
        .register(Arc::new(crate::engine::internal::InternalAdapter::new(
            state.clone(),
            runtime.db.clone(),
...
async fn cross_principal_rest_surface_is_disjoint() {
    ...
    let matrix: Vec<(&str, String, Option<serde_json::Value>)> = vec![
        ...
        (
            "POST",
            format!("/chats/{}/turns/{}/edit", chat.id, TurnId::new()),
            Some(serde_json::json!({"new_turn_id": TurnId::new(), "content": "mine now"})),
        ),
        (
            "POST",
            format!("/chats/{}/turns/{}/branch", chat.id, TurnId::new()),
            None,
        ),
...
fn deployment_plane_routes() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        // Adding from the directory saves a server every member's turns use.
        ("POST", "/mcp/directory/example/add"),
        ...
        (
            "DELETE",
            "/connected-apps/skipped/00000000-0000-0000-0000-000000000000",
        ),
