...
async fn create_app_bound_to(
    store: &Arc<dyn Store>,
    app: tidebreak_core::id::ConnectedAppId,
    operation_ids: &[&str],
) -> AppId {
...
async fn unknown_apps_and_unconfigured_bindings_refuse_on_the_grant_surface() {
    ...
    let unbound = create_app_bound_to(
        &store,
        tidebreak_core::id::ConnectedAppId::new(),
        &["listIssues"],
    )
    .await;
            ...
            revision: NewAppRevision {
                ...
                manifest: AppManifest {
                    ...
                    bindings: vec![AppBinding::Operations(
                        tidebreak_core::local_app::AppOperationsBinding {
                            app: connected,
...
async fn folder_bindings_grant_and_fail_closed_when_the_folder_disconnects() {
    ...
    impl HostFolders for FakeFolders {
        ...
        async fn list_folder(
            &self,
            _app: tidebreak_core::id::AppId,
            _root: tidebreak_core::id::HostRootId,
            _path: &str,
    ...
    let root = tidebreak_core::id::HostRootId::from_uuid(uuid::Uuid::new_v4()).unwrap();
