...
async fn external_parent_events_carry_authorized_child_trees() {
    ...
    // Read the existing tree before changing the child. That separates
    // reconnect replay from the quiet parent's subsequent live update.
    loop {
        let value = tokio::time::timeout(Duration::from_secs(5), next_external_json(&mut socket))
            .await
            .expect("initial tree replay must arrive");
        if value["event"]["type"] == "session_tree" {
            let replay = value["event"]["children"].as_array().unwrap();
            assert_eq!(replay.len(), 1, "{value}");
            assert_eq!(replay[0]["id"].as_str(), Some(visible_id_text.as_str()));
    ...
    let fence_reason = tidebreak_core::FenceReason::ResumeLost {
        detail: "The engine no longer has the child session".into(),
    };
    visible.fence_reason = Some(fence_reason.clone());
    ...
    let mut saw_reaped_child = false;
    for _ in 0..20 {
        let value = tokio::time::timeout(Duration::from_secs(5), next_external_json(&mut socket))
            .await
            .expect("clearing a child fault must update the parent tree");
        if value["event"]["type"] == "session_tree" {
            let children = value["event"]["children"].as_array().unwrap();
            assert_eq!(children.len(), 1, "{value}");
            assert_eq!(children[0]["id"].as_str(), Some(visible_id_text.as_str()));
            if children[0]["fenced"] == false {
                assert_eq!(children[0]["attention"], false, "{value}");
                saw_reaped_child = true;
                break;
            }
        }
    }
    assert!(
        saw_reaped_child,
        "the parent must clear the child's reap control"
    );
    ...
    let deleted = connection
        .execute_raw(sea_orm::Statement::from_sql_and_values(
            ...
            "DELETE FROM code_external_binding WHERE session_id = ?",
            [visible.id.0.into()],
        ))
        .await
        .unwrap()
