...
//! native process (`tidebreak-desktop`'s `client_execution::folder_operations`).
...
//! Standing consent still comes only from `tidebreak folder connect`. See
//! [`crate::folder`].
//!
...
//! the call for this process. Without a headless answer, `tidebreak serve` would
...
pub struct FolderExecutor {
    ...
    /// This profile's stable executor identity, shared with `tidebreak folder`.
...
impl FolderExecutor {
    ...
    /// time) and published through the same ingest route `tidebreak attach` and
    ...
    async fn import(
        ...
        let media_type =
            tidebreak_server::media_type::sniff_media_type(&bytes, Some(title.as_str()));
    ...
    /// return, so `host-broker.lock` is free for `tidebreak folder` provisioning
...
/// (`tidebreak-desktop/src/client_execution/folder_operations.rs`) field for
...
mod tests {
    use super::*;
    use tidebreak_core::{TurnId, REQUEST_FOLDER_ACCESS_TOOL};
