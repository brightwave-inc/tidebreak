...
/// Endpoint of a live `tidebreak serve` process.
...
/// Start `tidebreak serve` and return its base URL and bearer token.
fn start_serve(data_dir: &Path) -> ServeEndpoint {
    let mut child = tidebreak(data_dir)
        .arg("serve")
        .stdout(Stdio::piped())
        .spawn()
    ...
    let token = token_line
        .trim_start_matches("tidebreak: token ")
        .trim()
        .to_owned();
...
fn run(data_dir: &Path, args: &[&str]) -> (bool, String, String) {
    let output = tidebreak(data_dir)
        .args(args)
        .stdin(Stdio::null())
        .output()
        .expect("run tidebreak folder");
    (
...
fn connect_folder(data_dir: &Path, path: &Path, chat: &str) -> String {
    ...
    assert!(ok, "connect failed: {stderr}");
    // "tidebreak: connected <name> to chat <id> (root <root id>, operator …)"
    let root = stdout
        .split("(root ")
        .nth(1)
...
fn an_unattended_turn_reads_a_folder_the_operator_connected() {
    ...
    let output = tidebreak(&data_dir)
        ...
        .env("TIDEBREAK_SCRIPTED_PROVIDER", folder_reading_script(&root))
...
/// broker state. Here that process is `tidebreak serve`, and the run driving the
...
fn a_serve_daemon_executes_folder_calls_for_an_attached_client() {
    ...
    let mut daemon = tidebreak(&data_dir)
        .arg("serve")
        .env("TIDEBREAK_SCRIPTED_PROVIDER", folder_reading_script(&root))
        .env_remove("TIDEBREAK_SERVER_URL")
        .stdout(Stdio::piped())
    ...
    let attached = tidebreak(&data_dir)
        .args([
            "-p",
            "summarize the report",
            "--chat",
        ...
        .env_remove("TIDEBREAK_SERVER_URL")
        .env_remove("TIDEBREAK_SCRIPTED_PROVIDER")
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn an attached tidebreak -p");
...
fn a_serve_daemon_declines_folder_access_requests_for_an_attached_client() {
    ...
    let mut daemon = tidebreak(&data_dir)
        .arg("serve")
        .env("TIDEBREAK_SCRIPTED_PROVIDER", script)
        .env_remove("TIDEBREAK_SERVER_URL")
        .stdout(Stdio::piped())
        .spawn()
        .expect("spawn tidebreak serve");
...
/// The contract this change exists for: `tidebreak folder connect|list|
/// disconnect` must succeed while `tidebreak serve` holds `tidebreak.lock`.
...
fn folder_connect_while_serve_holds_the_data_directory() {
    ...
    assert!(
        data_dir.join("tidebreak.lock").exists(),
        "serve should hold tidebreak.lock"
    );
    let root = connect_folder(&data_dir, &reports, &chat);
    let (ok, listed, stderr) = run(&data_dir, &["folder", "list", "--chat", &chat]);
