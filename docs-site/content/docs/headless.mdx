...
while a turn runs, and while an ignored file or anything else no checkpoint
holds is in the way; the refusal names those files. A restore prints its id;
pass it to `--undo` to put back what the restore replaced.
