...
A debug build of the CLI, such as `cargo run -p tidebreak-cli --`, uses the
dev app's profile (`io.brightwave.tidebreak.dev`) in place of the installed
app's.
...
cargo run -p tidebreak-cli -- chat retry "$CHAT" --wait
cargo run -p tidebreak-cli -- chat regenerate "$CHAT" --wait
cargo run -p tidebreak-cli -- chat regenerate "$CHAT" --model anthropic::claude-opus-5
cargo run -p tidebreak-cli -- chat edit "$CHAT" "Make it shorter" --wait
BRANCH=$(cargo run -p tidebreak-cli -- chat branch "$CHAT")
```
...
```sh
cargo run -p tidebreak-cli -- data show
cargo run -p tidebreak-cli -- data backup ./tidebreak-backup.tar.gz
cargo run -p tidebreak-cli -- data export ./conversations.zip
cargo run -p tidebreak-cli -- data export ./chat.json --format json --chat <chat-uuid>
```

`show` prints the data folder and how much disk its database, attachments,
outputs, logs, engine tools, and backups use.

`backup` writes one `.tar.gz` with the database, the files attached to
conversations, and the files Tidebreak made. It copies the database with
SQLite's own `VACUUM INTO` while the server keeps running, so the copy is
consistent without stopping anything. Keys stay in the keychain and are never
in the archive. To restore, quit Tidebreak, move the data folder aside, and
extract the archive into an empty folder of the same name. A server on
PostgreSQL refuses `backup`: back that database up with its own tools, as
[Self-hosting](#self-hosting) describes.

`export` writes your conversations: a `.zip` with one Markdown file per
conversation, or one JSON document with `--format json`. Each `--chat` limits
the export to that conversation. Both write the file in full before they
replace anything at the path.
...
`.dev`). With `TIDEBREAK_DATA_DIR` unset, `--attach` reads the app's own
`listen.json`, so it fails when the app is not running. `--attach`, `--server`,
and `--embed` conflict; pick one.
...
```sh
TIDEBREAK_DATA_DIR="$(mktemp -d)" cargo run -p tidebreak-cli -- serve > server.log &
