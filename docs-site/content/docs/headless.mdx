...
- `edit` replaces the latest message and answers the new one. When the turn it
  replaces wrote files, created outputs, called connected apps, or started
  other work, the edit starts a new conversation instead, and says so on
  stderr. The original conversation keeps its record of what ran.
