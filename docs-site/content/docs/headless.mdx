...
Tests check each of these against the code, so a 1.x release cannot drop one
by accident:

- **Commands and flags.** Every command and flag in the
  [Commands](#commands) block. A 1.x release can add a command or a flag. It
  does not remove or rename one.
- **Exit codes.** The codes in the exit-code tables above.
- **JSON documents.** A command run with `--output-format json` or `--json`
  prints one object on one line, and every object carries
  `"schema_version": 1`. The version names the shape. A release can add keys
  under it; a change that removes, renames, or retypes a top-level key raises
  it. Read the keys you use and ignore the rest.

Some documents carry a record exactly as the server returned it, such as the
`settings` object in `settings show` or the snapshots the `code` family
prints. That record has the shape the HTTP API gives it, and the HTTP API is
internal.

### What is internal

These can change in any minor release. If you build on one, pin the Tidebreak
...
- The NDJSON streams from `-p --output-format json`, `code run --json`, and
  `code watch --json`. They carry the event wire line by line, along with the
  driver lines print mode writes.
