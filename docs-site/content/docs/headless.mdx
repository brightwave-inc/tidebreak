...
and undoes every change made since, including your own. `--turn N` names the
turn the way `code diff` does. Run it with `--dry-run` first to list what it
would undo. It refuses while a turn runs. A restore prints its id; pass it to
