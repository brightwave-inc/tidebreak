...
```sh
cargo run -p openwave-cli -- output list <chat-uuid>
cargo run -p openwave-cli -- output show <chat-uuid> <output-uuid>
cargo run -p openwave-cli -- output revisions <chat-uuid> <output-uuid>
cargo run -p openwave-cli -- output export <chat-uuid> <output-uuid> ./report.md
```
...
revision it read on stderr, so `openwave output show … > draft.md` is exactly
...
```sh
cargo run -p openwave-cli -- attach <chat-uuid> ./contract.pdf
```
