...
plugin installed from Git starts off — turn it on in the Plugins panel
before the agent can use its skills or launch any bundled MCP server.
...
The Plugins page can import one public HTTPS repository at a pinned tag or
full commit SHA. A moving branch is refused. The importer accepts a root
`plugin.json` (Agent Plugins), a `PLUGIN.md` bundle, or a lone `SKILL.md`
(the Claude / Agent Skills shape). The plugin's files run with the agent's
permissions. Members the importer skips are listed with the reason.

From the CLI:

```sh
tidebreak plugins install --git https://github.com/acme/notes --ref v1.0.0
tidebreak plugins install --git https://github.com/acme/notes --ref v1.0.0 --json
```

`--json` prints one object stamped with `schema_version`. There is no
marketplace, no auto-update, and no directory of plugins.
