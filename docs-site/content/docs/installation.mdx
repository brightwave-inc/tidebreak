...
The release app and the `tidebreak` command-line tool share one keychain item,
so the command line is signed out too. It leaves the worktrees in
`~/Tidebreak` and your repositories alone. Worktrees from before version 0.59
live inside the data directory, and the system dialog says so when it finds
any.
