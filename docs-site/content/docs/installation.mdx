---
title: Installation
description: Download OpenWave for macOS or Windows, or build it from source.
---

## macOS

Download the disk image and drag OpenWave to Applications.

[Download for macOS (Apple Silicon)](https://github.com/brightwave-inc/openwave/releases/latest/download/OpenWave-macos-apple-silicon.dmg)
...
Download the installer and run it. It installs per-user and needs no
administrator elevation.

[Download for Windows (x86_64)](https://github.com/brightwave-inc/openwave/releases/latest/download/OpenWave-windows-x86_64-setup.exe)

<Callout type="warn">
The Windows installer is not yet Authenticode-signed, so SmartScreen warns on
first run. Verify the download against its `.sha256` sidecar before you accept
that warning.
</Callout>

Windows builds do not currently run the in-app update loop. To move to a newer
version, download and run the newer installer.

## Linux

There is no prebuilt Linux package. Build from source.
...
OpenWave owns one application data directory, created on first launch:

| Platform | Path |
| --- | --- |
| macOS | `~/Library/Application Support/io.brightwave.openwave` |
| Windows | `%APPDATA%\io.brightwave.openwave` |

Inside it:

| Item | What it holds |
| --- | --- |
| `openwave.db` | The SQLite database: chats, messages, turns, event history, document metadata, and canonical text |
| `blobs/` | The original bytes of every document you attached |
| `openwave.lock` | Proof that one process owns this directory |

Provider API keys are **not** in that directory. They go to your OS credential
store (the login keychain on macOS) under the service name `openwave`.

<Callout type="warn">
While OpenWave is pre-1.0, opening a database written by an older build can
