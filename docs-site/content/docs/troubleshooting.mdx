...
## SmartScreen warns on the Windows installer

The Windows installer is not yet Authenticode-signed. Check the download
against its `.sha256` sidecar from the release page before accepting the
warning.
