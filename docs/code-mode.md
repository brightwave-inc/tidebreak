...
- Syntax color comes from highlight.js, the highlighter the transcript and
  ...
  end. A file over 10,000 diff lines, a hunk over 2,500 lines, and a hunk
  with a line over 1,000 characters stay plain.
...
- Split view is the reader's remembered choice. Below 800 pixels the view
  draws unified.
- A long diff mounts one chunk of 120 rows on its first frame and one more
...
each comment, the path, the new and old line spans, the quoted lines with
diff markers, and the comment. The send marks them while the request is out
and drops them once the server accepts it; a refusal leaves them pending.
The transcript reads the block back and folds it into one compact list.
