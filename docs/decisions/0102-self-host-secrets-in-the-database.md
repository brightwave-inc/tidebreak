...
- The file holds a 32-byte key as one line of standard base64; trailing
  ...
  could plant a key it knows. It warns, but starts, when its group or every
  account can read it. Group read access is how a container's server user
  shares the file with the host account that created it, so it is allowed,
  but the mode cannot say who belongs to the group, so the warning names the
  group and says it is safe only when the group holds just the accounts that
  run Tidebreak. The check applies to the file a symlink names, since
  Kubernetes mounts secrets through symlinks.
...
refused, readable by every account or by its group loaded with a warning
that says which, and a symlink judged by its target. The PostgreSQL lane runs the same flow against a fresh database and
