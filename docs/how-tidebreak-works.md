...
This is a maintainer's tour of Tidebreak: what the product is today, what happens
...
Both the desktop and `tidebreak serve` use the same server boot path. The native
...
legacy `TIDEBREAK_MCP_CONFIG` file supplies the initial definitions when no saved
...
contract before Tidebreak can claim exactly-once effects. The search consent
prompt explicitly describes that the query and potentially matching source
excerpts may be sent to configured AI services outside Tidebreak.
...
Tidebreak's operational scratch stays in private app storage; user-selected paths
...
`tidebreak serve` starts the same local HTTP/WebSocket server and prints the
...
request/response types, and handler documentation in `tidebreak-server` are the
effective API specification.

### MCP

`tidebreak mcp /absolute/workspace` starts an MCP stdio server confined to that
...
servers from the `TIDEBREAK_MCP_CONFIG` file used by the desktop and headless boot
...
- **The token names someone.** Self-host authenticates with static named
  bearer tokens from an operator file (`TIDEBREAK_AUTH_TOKENS_FILE`; format in
...
— a container publishing a port, say — sets `TIDEBREAK_LISTEN_ADDR` to a full
address and port (`0.0.0.0:8080`), and `tidebreak serve` announces the bound
...
| --- | --- | --- |
| Core contracts and model | `crates/tidebreak-core/src/lib.rs`, `model.rs`, `storage.rs` | Typed IDs, persisted records, store/tool/provider traits |
...
| MCP | `crates/tidebreak-mcp/src/`, `crates/tidebreak-cli/src/main.rs` | MCP protocol server and stdio command |
| Connectors | `crates/tidebreak-server/src/connectors` | Gateway and ChatGPT loopback OAuth (PKCE), owned by the server host |

The dependency direction is intentionally simple: clients compose libraries,
and libraries point down toward `tidebreak-core`. Core defines the contracts; it
